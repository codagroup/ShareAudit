...
public class FileSystemOperationService(ISidUtilitiesService sidUtilitiesService) : IFileSystemOperationService
{
    private readonly ISidUtilitiesService _sidUtilitiesService = sidUtilitiesService ?? throw new ArgumentNullException(nameof(sidUtilitiesService));
    ...
    public IEnumerable<AccessRuleEntry> EnumerateAccessRules(string path)
    {
        ...
        if ((attributes & FileAttributes.Directory) == FileAttributes.Directory)
        {
            try
            {
                ...
                authorizationRuleCollection = directoryInfo.GetAccessControl().GetAccessRules(true, true, typeof(SecurityIdentifier));
            }
        ...
        else
        {
            try
            {
                ...
                authorizationRuleCollection = fileInfo.GetAccessControl().GetAccessRules(true, true, typeof(System.Security.Principal.SecurityIdentifier));
            }
            catch (InvalidOperationException)
            {
            }
        ...
        if (isDirectory)
        {
            ...
            if (effectiveAccess.Read)
            {
                ...
                try
                {
                    ...
                    FileInfo file = new($"{path}\\write-test.shareaudit");
                    FileSystemAclExtensions.Create(file, FileMode.CreateNew, FileSystemRights.FullControl, FileShare.Read, 1, FileOptions.DeleteOnClose, fileSecurity);
                }
                catch (IOException)
                {
