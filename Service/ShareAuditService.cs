...
public sealed class ShareAuditService : IShareAuditService, IDisposable, INotifyPropertyChanged
{
    ...
    private void DoAuthenticatedHostWork(Host host)
    {
        ...
        {
            _uiContext.Send((_) => host.Shares = [], null);
    ...
    private void DoFileEntryWork(FileEntry fileEntry)
    {
        ...
        {
            _uiContext.Send((_) => fileEntry.AccessRules = [], null);
    ...
    private void DoFolderEntryWork(IFolderEntry folderEntry)
    {
        ...
        var level = folderEntry.FullName.Trim('\\').Split('\\').Length - 1;
        ...
        {
            _uiContext.Send((_) => folderEntry.AccessRules = [], null);
        ...
        {
            _uiContext.Send((_) => folderEntry.FileSystemEntries = [], null);
    ...
    private void DoProjectWork()
    {
        ...
        {
            _uiContext.Send((_) => _project.Hosts = [], null);
    ...
    private void DoWork(object? sender, DoWorkEventArgs e)
    {
        ...
        if (!_project.Configuration.Credentials.UseCurrentCredentials && _project.Configuration.UseAlternateAuthenticationMethod)
        {
            UserCredentials credentials = new(_project.Configuration.Credentials.Domain, _project.Configuration.Credentials.Username, _project.Configuration.Credentials.Password);
            try
            {
                WindowsIdentity.RunImpersonated(credentials.LogonUser(LogonType.NewCredentials), () => { DoProjectWork(); });
            }
            catch
            {
                throw;
